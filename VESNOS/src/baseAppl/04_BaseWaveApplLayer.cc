...
const simsignalwrap_t BaseWaveApplLayer::mobilityStateChangedSignal = simsignalwrap_t(SIMU_SIGNAL_MOBILITY_CHANGE_NAME);
...
void BaseWaveApplLayer::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        module = this->getParentModule()->getSubmodule("nic")->getSubmodule("LteMacBase");
...
omnetpp::simtime_t BaseWaveApplLayer::computeAsynchronousSendingTime(omnetpp::simtime_t interval)
{
    ...
    return firstEvent;*/
    omnetpp::simtime_t randomOffset = dblrand() * beaconInterval;
    omnetpp::simtime_t firstEvent = simTime() + randomOffset;
    return firstEvent;
}
