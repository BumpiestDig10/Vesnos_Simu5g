...
void ConstSpeedMobility::setTargetPosition()
{
    ...
    do
    {
        ...
        debugEV << "new targetPos: " << targetPos.str() << " distance=" << distance
    ...
    debugEV << "stepSize: " << stepSize.str() << " target: " << (stepSize*numSteps).str() << std::endl;
...
void ConstSpeedMobility::makeMove()
{
    ...
    if( step == numSteps )
    {
        ...
        debugEV << "stepping forward. step #=" << step
                << " startPos: " << move.getStartPos().str() << std::endl;
