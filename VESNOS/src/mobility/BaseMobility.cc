...
const simsignalwrap_t BaseMobility::mobilityStateChangedSignal = simsignalwrap_t(SIMU_SIGNAL_MOBILITY_CHANGE_NAME);
...
void BaseMobility::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        move.setStart(pos);
        coreEV << "start pos: " << move.getStartPos().str() << std::endl;
...
{
    coreEV << "start MOVE_TO_BORDER:" << move.str() << std::endl;
    ...
    case PLACERANDOMLY:
        move.setStart(bMsg->getStartPos());
        coreEV << "new random position: " << move.getStartPos().str() << std::endl;
        break;
    ...
    coreEV << "end MOVE_TO_BORDER:" << move.str() << std::endl;
}


void BaseMobility::updatePosition()
{
    EV << "updatePosition: " << move.str() << std::endl;
...
void BaseMobility::reflectCoordinate(BorderHandling border, inet::Coord& c)
{
    switch( border )
    {
    case X_SMALLER:
...
void BaseMobility::reflectIfOutside(BorderHandling wo, inet::Coord& stepTarget, inet::Coord& targetPos, inet::Coord& step, double& angle)
{
...
void BaseMobility::wrapIfOutside(BorderHandling wo, inet::Coord& stepTarget, inet::Coord& targetPos)
{
    switch( wo )
    {
    case X_SMALLER:
...
void BaseMobility::placeRandomlyIfOutside( inet::Coord& targetPos )
{
    targetPos = world->getRandomPosition();
}


BaseMobility::BorderHandling BaseMobility::checkIfOutside( inet::Coord targetPos, inet::Coord& borderStep )
{
    ...
    coreEV << "checkIfOutside, outside="<<outside<<" borderStep: " << borderStep.str() << std::endl;

    return outside;
}


void BaseMobility::goToBorder(BorderPolicy policy, BorderHandling wo, inet::Coord& borderStep, inet::Coord& borderStart)
{
    double factor;

    coreEV << "goToBorder: startPos: " << move.getStartPos().str()
    	                           << " borderStep: " << borderStep.str()
    	                           << " BorderPolicy: " << policy
    ...
    coreEV << "goToBorder: startPos: " << move.getStartPos().str()
    	                           << " borderStep: " << borderStep.str()
    	                           << " borderStart: " << borderStart.str()
    	                           << " factor: " << factor << std::endl;
}


bool BaseMobility::handleIfOutside(BorderPolicy policy, inet::Coord& stepTarget, inet::Coord& targetPos, inet::Coord& step, double& angle)
{
    ...
    coreEV << "handleIfOutside:stepTarget = " << stepTarget.str() << std::endl;
    ...
    coreEV << "old values: stepTarget: " << stepTarget.str()
    	                           << " step: " << step.str()
    	                           << " targetPos: " << targetPos.str()
    ...
    coreEV << "new values: stepTarget: " << stepTarget.str()
    	                           << " step: " << step.str()
    	                           << " angle: " << angle << std::endl;
    ...
    coreEV << "border handled, borderStep: "<< borderStep.str()
    	                           << "borderStart: " << borderStart.str()
    	                           << " stepTarget " << stepTarget.str() << std::endl;
