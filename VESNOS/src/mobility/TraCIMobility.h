...
#ifndef INET_MOBILITY_TRACI_TRACIMOBILITY_H
#define INET_MOBILITY_TRACI_TRACIMOBILITY_H
...
class TraCIMobilityMod : public inet::MobilityBase
{
protected:
    ...
    const static inet::simsignal_t parkingStateChangedSignal;

    bool isParking = false;

public:

    TraCIMobilityMod() : MobilityBase(), isPreInitialized(false) { }
    ~TraCIMobilityMod() { }
    virtual void initialize(int);
    virtual void finish();
    virtual void handleSelfMsg(omnetpp::cMessage *msg);

    virtual void preInitialize(std::string, const inet::Coord&, std::string = "", double = -1, double = -1);
    virtual void nextPosition(const inet::Coord&, std::string = "", double = -1, double = -1, VehicleSignal = VEH_SIGNAL_UNDEF);
    ...
    virtual inet::Coord getPositionAt(const omnetpp::simtime_t& t) const { return getPositionAt(t); }
