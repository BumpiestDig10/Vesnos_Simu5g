...
void TraCI_Commands::polygonAdd(std::string polyId, std::string polyType, const RGB color, bool filled, int32_t layer, const std::list<inet::Coord>& points)
{
...
void TraCI_Commands::poiAdd(std::string poiId, std::string poiType, const RGB color, int32_t layer, const inet::Coord& pos)
{
...
TraCICoord TraCI_Commands::obstacleGetPosition(std::string obstacleId)
{
    ...
    for(auto &obstacle : obstacle_list)
    {
        if(obstacle == obstacleId)
        {
            ...
            return convertCoord_omnet2traci(inet::Coord(xPos, yPos));
        }
    }
