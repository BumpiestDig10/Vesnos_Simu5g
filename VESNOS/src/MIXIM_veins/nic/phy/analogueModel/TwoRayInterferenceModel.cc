...
void TwoRayInterferenceModel::filterSignal(AirFrame *frame, const inet::Coord& senderPos, const inet::Coord& receiverPos)
{
    EV_STATICCONTEXT

	Signal& s = frame->getSignal();

	const inet::Coord senderPos2D(senderPos.x, senderPos.y);
	const inet::Coord receiverPos2D(receiverPos.x, receiverPos.y);
