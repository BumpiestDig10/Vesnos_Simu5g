...
const simsignal_t BaseWaveApplLayer::mobilityStateChangedSignal = registerSignal("mobilityChangedSignal");
...
void BaseWaveApplLayer::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        mobility = static_cast<MobilityBase *>(module);
        ASSERT(mobility);
        ...
        module = this->getParentModule()->getSubmodule("mac")->getSubmodule("LteMacBase");
        mac = static_cast<LteMacBase *>(module);
        ASSERT(mac);
        ...
        if (hasOBU) {
            //mac = check_and_cast<LteMacUe*>(inet::FindModule<LteMacUe>::findSubModule(getHostModule()));
            auto mac  = check_and_cast<LteMacUe*>(getContainingNode(this)->getSubmodule("mac"));
        }
        else {
            //mac = check_and_cast<LteMacEnb*>(inet::FindModule<LteMacEnb>::findSubModule(getHostModule()));
            auto mac  = check_and_cast<LteMacEnb*>(getContainingNode(this)->getSubmodule("mac"));
        }
        ...
        curPosition = inet::Coord(getParentModule()->getSubmodule("mobility")->par("x"), getParentModule()->getSubmodule("mobility")->par("y"));
        curSpeed = inet::Coord(0,0);
...
{
    auto mobility = dynamic_cast<inet::IMobility*>(obj);
...
omnetpp::simtime_t BaseWaveApplLayer::computeAsynchronousSendingTime(omnetpp::simtime_t interval)
{
    ...
    */
    ...
    double offset = uniform(0.0, interval.dbl());
    omnetpp::simtime_t firstEvent = simTime() + offset;
    return firstEvent;
}
