...
void AddNode::finish()
{
    ...
    //BaseConnection *cc = static_cast<BaseConnection*>(module);
    auto cc = inet::FindModule<ChannelControl*>::findGlobalModule();
    ASSERT(cc);

    /*auto deregisterAndDelete = [&](const std::pair<const std::string, adversaryEntry_t>& entry) {
            cModule* mod = entry.second.module;
            ASSERT(mod);

            // retrieve stored RadioRef
            auto radioRef = entry.second.radioRef;
            if (radioRef)
                cc->unregisterRadio(radioRef);

            mod->callFinish();
            mod->deleteModule();

            TraCI->removeMapping(entry.second.id_str);
    };*/
    ...
    for(auto &i : allAdversary)
    {
        ...
        auto radioRef = i.second.radioRef;
        if (radioRef)
            cc->unregisterRadio(radioRef);
...
void AddNode::addAdversary()
{
    ...
    for(auto &entry : allAdversary)
    {
        ...
        inet::Coord co = TraCI->convertCoord_traci2omnet(entry.second.pos);
