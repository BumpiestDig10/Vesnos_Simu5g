...
double sqrTorusDist(inet::Coord c, inet::Coord b, inet::Coord size) {
...
void BaseConnectionManager::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        inet::Coord dim((*playgroundSize) / maxInterferenceDistance);
        ...
        findDistance = inet::Coord(std::max(playgroundSize->x, maxInterferenceDistance),
        ...
        findDistance += inet::Coord(EPSILON, EPSILON, EPSILON);
        ...
        ccEV << "findDistance is " << findDistance.str() << std::endl;
    }
    else if (stage == 1)
    {
...
BaseConnectionManager::GridCoord BaseConnectionManager::getCellForCoordinate(const inet::Coord& c)
{
    return GridCoord(c, findDistance);
}


void BaseConnectionManager::updateConnections(int nicID, const inet::Coord* oldPos, const inet::Coord* newPos)
{
...
bool BaseConnectionManager::registerNic(cModule* nic, ChannelAccess* chAccess, const inet::Coord* nicPos)
{
    assert(nic != 0);
...
void BaseConnectionManager::updateNicPos(int nicID, const inet::Coord* newPos)
{
    ...
    inet::Coord oldPos = ItNic->second->pos;
