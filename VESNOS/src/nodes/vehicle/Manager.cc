...
#include "nodes/SCMS/bsmverify.h"
...
void ApplVManager::onMessageType(omnetpp::cMessage* msg)
{
    ...
    else if(msg->getKind() == TYPE_BEACON_BSM)
    {
        BSM* wsm = dynamic_cast<BSM*>(msg);
        ASSERT(wsm);

        bsm_setPayload = wsm->getPayload();
        pseudo_cert = wsm->getPseudonym_certificate();
        pca_cert = wsm->getPCA_certificate();
        pca_pub_x = wsm->getPublic_key();

        // Initialize variables
        ECPoint2 pca_pub("compressed-y-0", pca_pub_x);
        ...
        std::string pseudo_cert_tbs;

        boost::multiprecision::cpp_int R_x;
        boost::multiprecision::cpp_int s;

        tie(pub_recon, R_x, s, bsm_tbs, pseudo_cert_tbs) = Payload_decode(pseudo_cert, bsm_setPayload);

        bool res = BSMVerify_(R_x, s, bsm_tbs, pseudo_cert, pseudo_cert_tbs, pub_recon, pca_cert, pca_pub);

        if (res == true) {
            EV << "----------------------------- " << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "BSM from "<< wsm->getSender() <<" is successfully verified!" << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "----------------------------- " << std::endl;
            // Convert hex string back to bytes
            uint8_t *decodedBuffer = decode_BSM_fields(bsm_tbs);
            // Assign decodedBuffer values to BsmMessage fields
            BeaconVehicle* decodedwsm;
            memcpy(&decodedwsm, decodedBuffer, sizeof(&decodedwsm));

            EV << "------After decode--------- " << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "sender         : " << decodedwsm->getSender() << std::endl;
            EV << "senderType     : " << decodedwsm->getSenderType() << std::endl;
            EV << "recipient      : " << decodedwsm->getRecipient() << std::endl;
            EV << "pos x          : " << decodedwsm->getPos().x << std::endl;
            EV << "pos y          : " << decodedwsm->getPos().y << std::endl;
            EV << "pos z          : " << decodedwsm->getPos().z << std::endl;
            EV << "speed          : " << decodedwsm->getSpeed() << std::endl;
            EV << "accel          : " << decodedwsm->getAccel() << std::endl;
            EV << "maxDecel       : " << decodedwsm->getMaxDecel() << std::endl;
            EV << "lane           : " << decodedwsm->getLane() << std::endl;
            EV << "platoonID      : " << decodedwsm->getPlatoonID() << std::endl;
            EV << "platoonDepth   : " << decodedwsm->getPlatoonDepth() << std::endl;
            EV << "angle          : " << decodedwsm->getAngle() << std::endl;
            EV << "brakes         : " << decodedwsm->getBrakes() << std::endl;
            EV << "density        : " << decodedwsm->getDensity() << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "----------------------------- " << std::endl;
            ...
            EV << "----------------------------- " << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "ERROR: Failed to verify BSM from "<< wsm->getSender() << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "----------------------------- " << std::endl;
            EV << "----------------------------- " << std::endl;
        }
