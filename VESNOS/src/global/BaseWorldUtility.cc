...
void BaseWorldUtility::initialize(int stage)
{
    ...
    else if(stage == 1)
    {
        ...
        if(!inet::FindModule<ChannelControl*>::findGlobalModule())
...
void BaseWorldUtility::initializeIfNecessary()
{
    ...
    use2DFlag = par("use2D");
    playgroundSize = inet::Coord(par("playgroundSizeX").doubleValue(),
...
inet::Coord BaseWorldUtility::getRandomPosition()
{
    initializeIfNecessary();

    return inet::Coord(uniform(0, playgroundSize.x),
