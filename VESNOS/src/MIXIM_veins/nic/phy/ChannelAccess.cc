...
const simsignalwrap_t ChannelAccess::mobilityStateChangedSignal = simsignalwrap_t(SIMU_SIGNAL_MOBILITY_CHANGE_NAME);
...
ChannelAccess::prop_t ChannelAccess::calculatePropagationDelay(const NicEntry* nic)
{
    ...
    inet::Coord sendersPos  = senderModule->getMobilityModule()->getCurrentPosition();
    inet::Coord receiverPos = receiverModule->getMobilityModule()->getCurrentPosition();
...
void ChannelAccess::recordFrameTx(omnetpp::cPacket *msg /*AirFrame11p*/, omnetpp::cGate *gate, prop_t propDelay)
{
    ...
    {
        inet::AirFrame11p *frame = dynamic_cast<inet::AirFrame11p *>(msg);
        ASSERT(frame);
        ...
        if(mac)
        {
            ...
            if(appl)
            {
                inet::WaveShortMessage *appl_class = dynamic_cast<inet::WaveShortMessage *>(appl);
                ASSERT(appl_class);
