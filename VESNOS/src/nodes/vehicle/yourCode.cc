...
dataMsg* ApplVYourCode::generateData()
{
    ...
    // Create the packet
    auto pkt = new inet::Packet("genericData");
    // Create and configure your dataMsg payload
    auto payload = makeShared<dataMsg>();
    payload->setKind(TYPE_GENERIC_DATA);
    payload->setSender(SUMOID.c_str());
    payload->setTarget(3);
    // Add header+payload length
    payload->addBitLength(headerLength + beaconLengthBits);
    pkt->insertAtBack(payload);
    return pkt;
}


void ApplVYourCode::printControlInfo(omnetpp::cMessage* msg)
{
    if(!printCtrlData)
        return;
    ...
    */
    ...
    // Extract tags
    auto errorTag = pkt->findTag<inet::ErrorRateInd>();
    bool success = errorTag ? (errorTag->getPacketErrorRate() == 0) : true;

    auto powerTag = pkt->findTag<inet::SignalPowerInd>();
    double recvPower_dBm = powerTag ? powerTag->getPower().get() : NaN;

    auto snirTag = pkt->findTag<inet::SnirInd>();
    double snr = snirTag ? snirTag->getAverageSnir().get() : NaN;

    GLOG(SUMOID, "default") << boost::format("%.6f: Received '%s' %s")
        % omnetpp::simTime().dbl()
        % pkt->getName()
        % (success ? "successfully" : "with errors");

    GLOG(SUMOID, "default") << boost::format(
        "Power: %.3f dBm (%.14f mW), SNR: %.3f dB")
        % recvPower_dBm
        % pow(10.0, recvPower_dBm/10.0)
        % snr;
